	  ...
	  mp_result *result)
{
 ...
 OUTER_LOOP:
  ...
  if (qanylim) {
	...
	for (int i=0; i<m; i++, ij++) {
      ...
      if (lpegged && (sum > 0)) {
	int ij = j*ldfjac;
	for (int i=0; i<m; i++, ij++) fjac[ij] = 0;
      }
      if (upegged && (sum < 0)) {
	int ij = j*ldfjac;
	for (int i=0; i<m; i++, ij++) fjac[ij] = 0;
 ...
 L200:
  ...
  double pnorm = mp_enorm(nfree,wa3);
  ...
  double temp1 = mp_enorm(nfree,wa3)*alpha/fnorm;
 ...
 L300:
  ...
  if (result && (result->covar || result->xerror)) {
    ...
    if (result->covar) {
      ...
      for (int j=0; j<(npar*npar); j++) result->covar[j] = 0;
    ...
    if (result->xerror) {
      for (int j=0; j<npar; j++) result->xerror[j] = 0;
  ...
  if (result) {
    strcpy_s(result->version, sizeof(result->version), MPFIT_VERSION);
    ...
    if (result->resid) {
      for (int j=0; j<m; j++) result->resid[j] = fvec[j];
    }
  }
