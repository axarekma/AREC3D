...
int cyl_sirt(MPI_Comm comm, arecImage images, float *angles, arecImage *xcvol, float lam, int maxit,
             float tol) {
    double t0;
    ...
    while (iter <= maxit) {
        if (iter == 1) {
            if (restarts == 0) {
                status = arecBackProject2D(images, angles, nangles, &bvol);
            ...
            status = arecBackProject2D(projstack, angles, nangles, &pcylvol);
            if (status != 0) goto EXIT;
...
int cyl_sirt_SQ(MPI_Comm comm, arecImage images, float *angles, arecImage *xcvol, float lam,
                int maxit, float tol) {
    double t0;
    ...
    while (iter <= maxit) {
        if (iter == 1) {
            if (restarts == 0) {
                status = arecBackProject2D_SQ(images, angles, nangles, &bvol);
                if (status != 0) goto EXIT;
            }
            ...
            status = arecBackProject2D_SQ(projstack, angles, nangles, &pcylvol);
            if (status != 0) goto EXIT;
