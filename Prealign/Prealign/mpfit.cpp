...
static int mp_fdjac2(mp_func funct, int m, int n, int *ifree, int npar, double *x, double *fvec,
                     double *fjac, int ldfjac, double epsfcn, double *wa, void *priv, int *nfev,
                     double *step, double *dstep, int *dside, int *qulimited, double *ulimit,
                     int *ddebug, double *ddrtol, double *ddatol);
static void mp_qrfac(int m, int n, double *a, int lda, int pivot, int *ipvt, int lipvt,
                     double *rdiag, double *acnorm, double *wa);
static void mp_qrsolv(int n, double *r, int ldr, int *ipvt, double *diag, double *qtb, double *x,
                      double *sdiag, double *wa);
static void mp_lmpar(int n, double *r, int ldr, int *ipvt, int *ifree, double *diag, double *qtb,
...
int mpfit(mp_func funct, int m, int npar, double *xall, mp_par *pars, mp_config *config,
    ...
    // GCC complains about cross-initialization
    // Declare variables here to fix
    // Axel Ekman 2018
    double temp, temp1, temp2, sum;
    double ratio, dirder, prered, actred, pnorm, alpha, gnorm, orignorm, par;
    int jj, iter, j;
    ...
    if (funct == 0) { return MP_ERR_FUNC; }

    if ((m <= 0) || (xall == 0)) { return MP_ERR_NPOINTS; }

    if (npar <= 0) { return MP_ERR_NFREE; }
...
OUTER_LOOP:
    ...
    iflag =
        mp_fdjac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac, conf.epsfcn, wa4,
                  private_data, &nfev, step, dstep, mpside, qulim, ulim, ddebug, ddrtol, ddatol);
    if (iflag < 0) { goto CLEANUP; }
    ...
    if (iter == 1) {
        if (conf.douserscale == 0) {
            for (int j = 0; j < nfree; j++) {
                diag[ifree[j]] = wa2[j];
                if (wa2[j] == zero) { diag[ifree[j]] = one; }
            }
        }
...
L200:
    ...
    if (qanylim == 0) {
        ...
        for (int j = 0; j < nfree; j++) {
            ...
            if (qulim[j] && (wa2[j] >= ulim1)) { wa2[j] = ulim[j]; }
            if (qllim[j] && (wa2[j] <= llim1)) { wa2[j] = llim[j]; }
        }
    }
     ...
     */
    if (iter == 1) { delta = mp_dmin1(delta, pnorm); }
    ...
    if (delta <= conf.xtol * xnorm) { info = MP_OK_PAR; }
    if ((fabs(actred) <= conf.ftol) && (prered <= conf.ftol) && (p5 * ratio <= one) &&
        (info == 2)) {
        info = MP_OK_BOTH;
    }
    ...
    if (delta <= MP_MACHEP0 * xnorm) { info = MP_XTOL; }
    if (gnorm <= MP_MACHEP0) { info = MP_GTOL; }
    if (info != 0) { goto L300; }
...
L300:
    ...
    if (result) {
        strncpy(result->version, MPFIT_VERSION, sizeof(result->version));
...
static int mp_fdjac2(mp_func funct, int m, int n, int *ifree, int npar, double *x, double *fvec,
    ...
    if (has_numerical_deriv)
        for (j = 0; j < n; j++) { /* Loop thru free parms */
            ...
            if ((dside && dsidei == -1) || (dside && dsidei == 0 && qulimited && ulimit &&
                                            qulimited[j] && (temp > (ulimit[j] - h)))) {
                h = -h;
            }
