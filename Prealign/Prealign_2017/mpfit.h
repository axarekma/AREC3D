...
struct mp_config_struct {
	...
	double ftol = 1e-10;        /* Relative chi-square convergence criterium */
	double xtol = 1e-10;;       /* Relative parameter convergence criterium  */
	double gtol = 1e-10;;       /* Orthogonality convergence criterium       */
	double epsfcn = MP_MACHEP0; /* Finite derivative step size               */
	double stepfactor = 100;    /* Initial step bound                        */
	double covtol = 1e-14;      /* Range tolerance for covariance calculation */
	int maxiter = 200;          /* Maximum number of iterations.  If maxiter == 0,
					then basic error checking is done, and parameter
					errors/covariances are estimated based on input
					parameter values, but no fitting iterations are done. */
	int maxfev = 0;      /* Maximum number of function evaluations, or 0 for no limit */
	int nprint = 1;      /* Default: 1 */
	int douserscale = 0; /* Scale variables by user values?
					1 = yes, user scale values in diag;
					0 = no, variables scaled internally  */
	int nofinitecheck = 0; /* Disable check for infinite quantities from user?
					   0 = do not perform check 
					   1 = perform check   */
	mp_iterproc iterproc = 0; /* Placeholder pointer - must set to 0 */
